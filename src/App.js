...
function App() {
  ...
  const copyPasswordToClipboaard = useCallback(() => {
    passwordRef.current?.select();
    passwordRef.current?.setSelectionRange(0, 35);
    window.navigator.clipboard.writeText(password)
  }, [password])

  useEffect(()=>{passwordGenrater()} , [length,numberAllowed, charAllowed,passwordGenrater])

  return (
    <div className='w-full max-w-md mx-auto shadow-md rounded-lg px-5 h-96 py-10 my-12 bg-gray-700 text-orange-500'>
      <h1 className='text-white text-center my-3 text-3xl' >Password Generator</h1>

      <div className="flex shadow rounded-lg overflow-hidden mb-4 h-9">
        ...
        <button onClick={copyPasswordToClipboaard} className='outline-none bg-blue-700 text-white px-3 py-0.5 shrink-0 text-xl'>Copy</button>
      </div>
